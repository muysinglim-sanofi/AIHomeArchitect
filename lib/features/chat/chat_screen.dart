  ...
  Future<void> _loadMessages({String trigger = 'unknown'}) async {
    ...
    try {
      ...
      if (trigger == 'poll') {
        ...
        // ── PR2b — Fenêtre A recovery: intended V1 never produced ────────────
        // A NEW session uploads a source photo then auto-generates V1. If the
        // user left DURING _initNewSession (before _triggerAutoGenerate fired),
        // the session exists with a source but NO vision AND no backend Intent.
        // On session open, DERIVE that from the backend and produce the intended
        // V1 now.
        //   G1 — only on a CONFIRMED "no intent" (probe != null, i.e. the request
        //        SUCCEEDED, AND intent_id == null). A failed probe (probe == null,
        //        network/backend down) NEVER triggers a blind auto-gen.
        //   G2 — bounded: per-instance (_hasAutoGeneratedInitialVision, checked
        //        inside _triggerAutoGenerate) + static _v1RecoveryFired (≤ once per
        //        session per app-launch). Once the auto-gen reaches the backend an
        //        Intent exists → the probe stops reporting "no intent" (durable).
        //   Idempotent/claim-safe: if an Intent actually exists (rare race), the
        //   atomic claim replays/202s (Slice 1) → never a double. A FAILED intent
        //   (intent_id != null) is NOT auto-retried.
        ...
        if (noIntentConfirmed &&
            ...
            !_v1RecoveryFired.contains(_project.id)) {
          _v1RecoveryFired.add(_project.id);
          debugPrint('[Recovery] session=${_project.id} — backend confirms NO intent, '
              'source present, no vision → producing intended V1 (Fenêtre A)');
          _triggerAutoGenerate();
          // The auto-gen now OWNS the UI: _triggerAutoGenerate → _generate runs
          // synchronously up to its network await, adding a loading bubble via
          // setState. RETURN here so the rest of _loadMessages doesn't re-render
          // _messages from the DB rows and CLOBBER that bubble — which would leave
          // the ~45s generation with no loading indicator, then pop the image.
          return;
        }
      }
