...
class _ChatScreenState extends ConsumerState<ChatScreen>
  ...
  // PR2b — sessions where the "intended V1 never produced" recovery (Fenêtre A)
  // has already fired this app-launch. Static → survives ChatScreen recreation;
  // reset only on app process kill. Bounds the recovery to at most ONCE per
  // session per launch (guardrail G2), so a failing/slow auto-gen can never loop
  // on every _loadMessages. Once the recovery reaches the backend, an Intent
  // exists → the probe stops reporting "no intent" anyway (durable bound).
  static final Set<String> _v1RecoveryFired = <String>{};
  ...
  Future<void> _initNewSession() async {
    ...
    try {
      ...
      // Bug B (2026-07-08) — persist the durable V1 auto-gen INTENTION NOW: the
      // session is real and the source is uploaded, and this runs BEFORE the greeting
      // await (the disposal window) / the !mounted abort in _triggerAutoGenerate /
      // the POST. If the user leaves during the init window, the boot recovery sweep
      // relaunches it. No-op when no source (upload failed → nothing to generate).
  ...
  /// Bug B (2026-07-08) — persist the durable V1 auto-gen INTENTION the moment it is
  /// valid (real session + source uploaded), BEFORE the greeting await / any
  /// `!mounted` abort in _triggerAutoGenerate / the POST. If the user leaves during
  /// the ~7s new-session init window, the boot recovery sweep sees "pending + no
  /// backend intent" → relaunches exactly once (backend dedups by deterministic
  /// intent_id → never a double gen). Orchestration ONLY: no composer / DNA / prompt
  /// / classifier / preservation. Idempotent by sessionId; _generate's own
  /// post-tuple save overwrites this verbatim (same clientRequestId) when it runs.
  Future<void> _savePendingV1AutoGen(String sessionId, String source) async {
    final store = _pendingStore;
    if (store == null) return;
    // Same logical key + request-id formula as _generate → the eventual _generate
    // reuses this SAME clientRequestId (putIfAbsent), so no divergence, no double.
    final genKey = '$source|1|$_genAttempt';
    final clientRequestId =
        _requestIdForKey.putIfAbsent(genKey, () => _newRequestId());
    final pending = buildAutoGenV1Pending(
      sessionId: sessionId,
      source: source,
      iterationCount: _iterationCount,
      alreadyAutoGenerated: _hasAutoGeneratedInitialVision,
      nowMs: DateTime.now().millisecondsSinceEpoch,
      clientRequestId: clientRequestId,
      prompt: _pendingDescription ??
          'Generate the first architectural vision for this space.',
      currentStyle: _currentStyle,
      currentRoomType: _currentRoomType,
      letAiDecide: _letAiDecide,
      ...
      versions: _versions,
      generationMode: _generationMode,
      // ref is invalid after dispose; guard so this works even mid-init teardown.
      uiLocale: mounted ? ref.read(localeProvider).languageCode : 'en',
      generationAttempt: _genAttempt,
    );
    if (pending == null) {
      debugPrint('[PendingV1] skipped reason=not_eligible '
          'session=$sessionId hasSource=${source.isNotEmpty} iter=$_iterationCount');
      return;
    }
    try {
      await store.save(pending);
      debugPrint('[PendingV1] saved before POST session=$sessionId key=$genKey');
    } catch (e) {
      debugPrint('[PendingV1] save failed (non-fatal): $e');
    }
  }
  ...
  Future<void> _loadMessages({String trigger = 'unknown'}) async {
    ...
    try {
      ...
      if (trigger == 'poll') {
        ...
        // ── PR2b — recover a "no vision, no backend intent" session ──────────
        // Only on a CONFIRMED "no intent" (probe SUCCEEDED, intent_id == null — a
        // FAILED probe never acts). Two DISJOINT paths, so a session is NEVER
        // re-launched twice:
        //   • a durable PENDING exists (Slice 2) → the recovery service RE-LAUNCHES
        //     it VERBATIM (Slice 3, guarded + backend-first); show loading + poll.
        //   • NO pending (Fenêtre A: user left during _initNewSession before the
        //     auto-gen fired) → produce a FRESH V1 via _triggerAutoGenerate.
        // Idempotent/claim-safe: PR2a's atomic claim dedups. A FAILED intent
        // (intent_id != null) is NOT auto-retried. G2-bounded (per-instance flag +
        // static _v1RecoveryFired ; once an intent exists the probe stops saying
        // "no intent").
        final noIntentConfirmed = probe != null && probe['intent_id'] == null;
        if (noIntentConfirmed &&
            imageCount == 0 &&
            !_isGenerating &&
            !_hasAutoGeneratedInitialVision) {
          ...
          if (pending != null) {
            ...
            debugPrint('[Recovery] session=${_project.id} — pending + no intent → '
                'sweep (verbatim re-launch) + loading');
            PendingRecoveryService.instance.sweep(reason: 'session-open');
            setState(() {
              _isGenerating = true;
              _messages = [
                ...msgs,
                MessageModel(
                  id: 'loading_recovery',
                  content: '${_iterationCount + 1}|$_currentStyle',
                  isAi: true,
                  type: MessageType.loading,
                  createdAt: DateTime.now(),
                ),
          ...
          // No pending → Fenêtre A: the auto-V1 never fired. Produce a fresh V1.
          final hasSource = (_project.beforeImageUrl ?? '').isNotEmpty;
          if (hasSource && !_v1RecoveryFired.contains(_project.id)) {
            _v1RecoveryFired.add(_project.id);
            debugPrint('[Recovery] session=${_project.id} — no pending, source, '
                'no vision → produce fresh V1 (Fenêtre A)');
            _triggerAutoGenerate();
            // The auto-gen OWNS the UI (adds a loading bubble synchronously up to
            // its network await). RETURN so _loadMessages doesn't re-render and
            // clobber that bubble.
            return;
          }
        }
