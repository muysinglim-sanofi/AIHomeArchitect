...
| 5.5.26 | Soft Luxury vessel `"oversized"` → `"floor-level"` + Dark Contemporary artwork → `"deep dark velvet throw layered on the sofa"` | LOW | **PASS** — bench validated by user 2026-05-25 | ✓ **SHIPPED.** Floor-level / sofa-anchored decor swaps work where wall-occupying decor blocked TV. Pattern usable for similar wall-competition fixes. Lean decor_language (1-2 items, geometry-anchored) confirmed as part of WM reference profile |
